...
Shader "Rito/Soft Particle"
{
	Properties
	{
		...
		[ASEBegin]_Noise5("Noise5", 2D) = "white" {}
		_Intensity("Intensity", Range( 0 , 5)) = 1
		_Noise17("Noise17", 2D) = "white" {}
		_T_VFX_Trail_03("T_VFX_Trail_03", 2D) = "white" {}
		_SoftParticleFactor("Soft Particle Factor", Range( 0 , 0.999)) = 0.5
		_Texturesize("Texture size", Vector) = (0.5,0.5,0,0)
		_Texturesize2("Texture size2", Vector) = (0.5,0.5,0,0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float2 texCoord41 = IN.ase_texcoord3.xy * _Texturesize + float2( 0,0 );
				float2 panner42 = ( 1.0 * _Time.y * float2( 0.2,0 ) + texCoord41);
				float2 texCoord43 = IN.ase_texcoord3.xy * _Texturesize2 + float2( 0,0 );
				float2 panner44 = ( 1.0 * _Time.y * float2( 0.2,0 ) + texCoord43);
				float2 texCoord36 = IN.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float2 panner40 = ( 1.0 * _Time.y * float2( 1,0 ) + texCoord36);
				float4 temp_output_23_0 = ( tex2D( _Noise17, ( float4( panner42, 0.0 , 0.0 ) + tex2D( _Noise5, panner44 ) ).rg ) * tex2D( _T_VFX_Trail_03, panner40 ) );
				
				float4 screenPos = IN.ase_texcoord4;
				float4 ase_screenPosNorm = screenPos / screenPos.w;
				ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
				float eyeDepth6 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy ),_ZBufferParams);
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = ( _Intensity * temp_output_23_0 * IN.ase_color * _Color ).rgb;
				float Alpha = ( temp_output_23_0 * IN.ase_color.a * saturate( ( ( 1.0 - _SoftParticleFactor ) * ( eyeDepth6 - screenPos.w ) * 3.0 ) ) ).r;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 texCoord41 = IN.ase_texcoord2.xy * _Texturesize + float2( 0,0 );
				float2 panner42 = ( 1.0 * _Time.y * float2( 0.2,0 ) + texCoord41);
				float2 texCoord43 = IN.ase_texcoord2.xy * _Texturesize2 + float2( 0,0 );
				float2 panner44 = ( 1.0 * _Time.y * float2( 0.2,0 ) + texCoord43);
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				float2 texCoord41 = IN.ase_texcoord.xy * _Texturesize + float2( 0,0 );
				float2 panner42 = ( 1.0 * _Time.y * float2( 0.2,0 ) + texCoord41);
				float2 texCoord43 = IN.ase_texcoord.xy * _Texturesize2 + float2( 0,0 );
				float2 panner44 = ( 1.0 * _Time.y * float2( 0.2,0 ) + texCoord43);
				...
				surfaceDescription.Alpha = ( temp_output_23_0 * IN.ase_color.a * saturate( ( ( 1.0 - _SoftParticleFactor ) * ( eyeDepth6 - screenPos.w ) * 3.0 ) ) ).r;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				float2 texCoord41 = IN.ase_texcoord1.xy * _Texturesize + float2( 0,0 );
				float2 panner42 = ( 1.0 * _Time.y * float2( 0.2,0 ) + texCoord41);
				float2 texCoord43 = IN.ase_texcoord1.xy * _Texturesize2 + float2( 0,0 );
				float2 panner44 = ( 1.0 * _Time.y * float2( 0.2,0 ) + texCoord43);
...
Node;AmplifyShaderEditor.RangedFloatNode;9;-1287.459,382.4543;Inherit;False;Property;_SoftParticleFactor;Soft Particle Factor;4;0;Create;True;0;0;0;False;0;False;0.5;0.509;0;0.999;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;5;-497.6649,223.7216;Inherit;False;3;3;0;COLOR;0,0,0,0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;4;-497.965,80.72173;Inherit;False;4;4;0;FLOAT;0;False;1;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;1;COLOR;0
...
ASEEND*/
//CHKSM=7FDE539F8C2A18FAEFCEE10B2DF6B1FAA56BC165