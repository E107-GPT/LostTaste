...
Shader "Fireball"
{
	Properties
	{
		...
		[ASEBegin]_Firefresenl("Fire fresenl", Range( 0 , 5)) = 0.78
		_TextureSample0("Texture Sample 0", 2D) = "white" {}
		_TextureSample1("Texture Sample 1", 2D) = "white" {}
		_Float0("Float 0", Range( 0 , 5)) = 0.06
		_FresenlEmission("Fresenl Emission", Range( 0 , 5)) = 3.55
		_Lerp("Lerp", Range( 0 , 1)) = 0.75
		_Color("Color ", Color) = (1,0,0,1)
		_MainEmission("Main Emission", Range( 0 , 5)) = 4.53
		_co("co", 2D) = "white" {}
		_me("me", 2D) = "white" {}
		[ASEEnd]_no("no", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float fresnelNode10 = ( 0.0 + 1.0 * pow( 1.0 - fresnelNdotV10, _Firefresenl ) );
				float2 texCoord16 = IN.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
				float2 panner15 = ( 1.0 * _Time.y * float2( 0.2,0.2 ) + texCoord16);
				float2 texCoord18 = IN.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
				float2 panner17 = ( 1.0 * _Time.y * float2( 0,0 ) + texCoord18);
				float4 temp_output_24_0 = ( fresnelNode10 * tex2D( _TextureSample1, ( float4( panner15, 0.0 , 0.0 ) + ( tex2D( _TextureSample0, panner17 ) * _Float0 ) ).rg ) * _FresenlEmission );
				float4 temp_cast_4 = (fresnelNode10).xxxx;
				float4 lerpResult66 = lerp( temp_cast_4 , temp_output_24_0 , _Lerp);
				float4 temp_output_31_0 = ( fresnelNode10 * temp_output_24_0 * _MainEmission * _Color * lerpResult66 );
				
				float2 uv_me = IN.ase_texcoord8.xy * _me_ST.xy + _me_ST.zw;
				

				float3 BaseColor = tex2D( _co, uv_co ).rgb;
				float3 Normal = tex2D( _no, uv_no ).rgb;
				float3 Emission = ( temp_output_31_0 * IN.ase_color ).rgb;
				float3 Specular = 0.5;
				float Metallic = tex2D( _me, uv_me ).r;
				float Smoothness = 0.5;
				float Occlusion = 1;
				float Alpha = ( temp_output_31_0 * IN.ase_color.a ).r;
		...
		Pass
		{
						 ...
						 ) : SV_TARGET
			{
				...
				float2 panner17 = ( 1.0 * _Time.y * float2( 0,0 ) + texCoord18);
				float4 temp_output_24_0 = ( fresnelNode10 * tex2D( _TextureSample1, ( float4( panner15, 0.0 , 0.0 ) + ( tex2D( _TextureSample0, panner17 ) * _Float0 ) ).rg ) * _FresenlEmission );
				float4 temp_cast_2 = (fresnelNode10).xxxx;
				float4 lerpResult66 = lerp( temp_cast_2 , temp_output_24_0 , _Lerp);
				float4 temp_output_31_0 = ( fresnelNode10 * temp_output_24_0 * _MainEmission * _Color * lerpResult66 );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 panner17 = ( 1.0 * _Time.y * float2( 0,0 ) + texCoord18);
				float4 temp_output_24_0 = ( fresnelNode10 * tex2D( _TextureSample1, ( float4( panner15, 0.0 , 0.0 ) + ( tex2D( _TextureSample0, panner17 ) * _Float0 ) ).rg ) * _FresenlEmission );
				float4 temp_cast_3 = (fresnelNode10).xxxx;
				float4 lerpResult66 = lerp( temp_cast_3 , temp_output_24_0 , _Lerp);
				float4 temp_output_31_0 = ( fresnelNode10 * temp_output_24_0 * _MainEmission * _Color * lerpResult66 );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				surfaceDescription.Alpha = ( temp_output_31_0 * IN.ase_color.a ).r;
...
WireConnection;57;2;35;0
WireConnection;57;3;68;0
WireConnection;57;6;33;0
WireConnection;66;0;10;0
WireConnection;66;1;24;0
WireConnection;66;2;27;0
ASEEND*/
//CHKSM=AC87C3DF86D4146F6B27BE25C88F680FEB745885